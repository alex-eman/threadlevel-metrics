...
/* Edited for ThreadLevel-Metrics */
vector<string> split(const char *event_name_list, string delim){

	vector<string> list;
	int length = strlen(event_name_list);
	size_t pos = 0;
	string orig = event_name_list;
	string token;

	while ((pos = orig.find(delim)) != string::npos) {
		token = orig.substr(0,pos);
		list.push_back(token);
		orig.erase(0, pos + delim.length());
	}
...
void PAPIEventSet::add_from_environment(const string& env_var, const char *default_events){
   ...
   vector<string> env_event_names = split(event_name_list, ", ");
