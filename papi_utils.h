...
std::vector<std::string> split(const char *event_name_list, std::string delim);
...
class PAPIEventSet {
	...
	public:
		...
		/// Create an empty event set.
   		...
   		///
   		void add_from_environment(const std::string& env_var,const char *default_events = NULL);
